...
const count = (target, input) => {
  ...
  let countNum = array.filter((element) => parseInt(element) === target);
...
const totalSum = (input) => {
  // Solution code here...
  let array = input.join(',').split(',');
  let sum = array.reduce((accumulator, currentValue) => parseInt(accumulator) + parseInt(currentValue));
  console.log(sum);
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArr = input.map((element) => {
    return element.filter((elem1) => {
      if ((!isNaN(elem1)) && elem1 === parseInt(elem1)) {
        return (elem1 % 5 === 0)
      }
    })
  })

  let finArr = newArr.map(element => {
    return element.map(elm => 2 ** elm)
  })
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let filtered = data.filter(element => ((element.gender === 'female') || (element.gender === 'male')))
    .map(elm => elm.name)
    .join(' and ')
...
let findShortest = (data) => {
  // Solution code here...
  let sortedHeights = data.sort((a,b) => a.height-b.height)

  return sortedHeights[0].name;
