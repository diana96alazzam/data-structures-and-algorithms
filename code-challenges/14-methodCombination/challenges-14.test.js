...
const toTitleCase = (arr) => {
  ...
  let newArr = arr.map(item => {
      return item = item.substring(0,1).toUpperCase()+item.substring(1,item.length);
  })
  return newArr;
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  let sorted = arr.sort((a,b)=> a.property>b.property)
  console.log(sorted);
  return sorted;
};
...
Write a function named  that accepts a two-dimensional array of strings. Each string is guaranteed to be either "X", "O" or an empty string. Your function should check to see if any row, column, or either diagonal direction has three matching "X" or "O" symbols (non-empty strings), three-in-a-line.
